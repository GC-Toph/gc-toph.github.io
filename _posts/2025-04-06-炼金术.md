...
bbb_data = load_data_from_cache_or_download("BAMLC0A4CBBB", start_date, end_date, data_type="fred")
...
merged_data = pd.DataFrame({
    "BAMLH0A3HYC": ccc_data["BAMLH0A3HYC"],  # 高收益债券利差
    "BAMLC0A4CBBB": bbb_data["BAMLC0A4CBBB"],  # BBB级债券利差
...
# 创建第二个Y轴，用于绘制 BAMLH0A1HYBB 和 BAMLH0A3HYC
ax2 = ax1.twinx()
line1, = ax2.plot(merged_data.index, merged_data["BAMLC0A4CBBB"], color="green", label="BBB")
line2, = ax2.plot(merged_data.index, merged_data["BAMLH0A3HYC"]/6, color="red", label="CCC / 6")
ax2.set_ylabel("High Yield Spread (%)", color="black")  # 使用黑色表示次轴的标签
ax2.tick_params(axis="y", labelcolor="black")

# 添加标题和图例
plt.title("Log_10_SP500 vs High Yield Spreads (BBB & CCC/6)")
...
# 显示图表
plt.savefig("Log_SP500_vs_BBB_CCC.png")
plt.show()
...
def percentile_today(data):
    today = data.iloc[-1]  # 最新数据点
    p = (data < today).mean() * 100 
    print(p, "%")

print("\n今日值占历史百分位:")
percentile_today(bbb_data)
percentile_today(ccc_data)
```
...
```text
基本统计信息:
       BAMLH0A3HYC  BAMLC0A4CBBB        ^GSPC  Log_10_SP500
count  7108.000000   7108.000000  7108.000000   7108.000000
mean     11.061495      1.923292  2036.993013      3.240828
std       5.103792      1.003424  1265.976255      0.232467
min       4.140000      0.720000   676.530029      2.830287
25%       7.690000      1.307500  1161.142548      3.064886
50%       9.530000      1.700000  1416.180054      3.151118
75%      12.590000      2.210000  2642.247498      3.421973
max      44.290000      8.040000  6144.149902      3.788462

今日值占历史百分位:
BAMLC0A4CBBB    24.481497%
BAMLH0A3HYC    56.486377%
```

![运行结果](/assets/images/20250406/Log_SP500_vs_BBB_CCC.png)
